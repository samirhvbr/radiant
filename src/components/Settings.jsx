...
const GUIDE = [
  {
    ...
    items: [
      ...
      ['Graph \u2014 choose a folder and see the shape of it', 'The new Graph tab reads every source file in a folder and draws what actually depends on what: a box is a folder that exists, an arrow is an import that resolves to a file on disk. Nothing is guessed \u2014 no model draws this, which is the point, because a diagram you have to check by hand is worse than no diagram. Press Choose folder, then Draw it. Choosing a file instead draws its folder file by file, so you can see what that one file pulls in. It also tells you what the project is written in, what it leans on most, and which folders nothing imports. Built and minified files are left out, so a bundled copy of an app is not mistaken for the app. Save the picture as an SVG, or press Explain this to open a chat about the drawing it just made.'],
